...
var strsv = require( './../lib/strsv.js' );
...
tape( 'the function has an arity of 9', function test( t ) {
	t.strictEqual( strsv.length, 9, 'returns expected value' );
	t.end();
});
...
tape( 'the function throws an error if provided an invalid first argument', function test( t ) {
	...
	function badValue( value ) {
		return function badValue() {
			strsv( value, data.uplo, data.trans, data.diag, data.N, new Float32Array( data.A ), data.LDA, new Float32Array( data.x ), data.strideX );
		};
	}
});
...
tape( 'the function throws an error if provided an invalid second argument', function test( t ) {
	...
	function badValue( value ) {
		return function badValue() {
			strsv( data.order, value, data.trans, data.diag, data.N, new Float32Array( data.A ), data.LDA, new Float32Array( data.x ), data.strideX );
		};
	}
});
...
tape( 'the function throws an error if provided an invalid third argument', function test( t ) {
	...
	function badValue( value ) {
		return function badValue() {
			strsv( data.order, data.uplo, value, data.diag, data.N, new Float32Array( data.A ), data.LDA, new Float32Array( data.x ), data.strideX );
		};
	}
});
...
tape( 'the function throws an error if provided an invalid fourth argument', function test( t ) {
	...
	function badValue( value ) {
		return function badValue() {
			strsv( data.order, data.uplo, data.trans, value, data.N, new Float32Array( data.A ), data.LDA, new Float32Array( data.x ), data.strideX );
		};
	}
});
...
tape( 'the function throws an error if provided an invalid fifth argument', function test( t ) {
	...
	function badValue( value ) {
		return function badValue() {
			strsv( data.order, data.uplo, data.trans, data.diag, value, new Float32Array( data.A ), data.LDA, new Float32Array( data.x ), data.strideX );
		};
	}
});
...
tape( 'the function throws an error if provided an invalid seventh argument', function test( t ) {
	...
	function badValue( value ) {
		return function badValue() {
			strsv( data.order, data.uplo, data.trans, data.diag, data.N, new Float32Array( data.A ), value, new Float32Array( data.x ), data.strideX );
		};
	}
});
...
tape( 'the function throws an error if provided an invalid ninth argument', function test( t ) {
	...
	function badValue( value ) {
		return function badValue() {
			strsv( data.order, data.uplo, data.trans, data.diag, data.N, new Float32Array( data.A ), data.LDA, new Float32Array( data.x ), value );
		};
	}
});
...
tape( 'the function solves one of the systems of equations `A*x = b` or `A^T*x = b` (row-major, lower, no transpose, non-unit)', function test( t ) {
	...
	out = strsv( data.order, data.uplo, data.trans, data.diag, data.N, a, data.LDA, x, data.strideX );
...
tape( 'if `N` is zero, the function returns the input vector unchanged (row-major)', function test( t ) {
	...
	out = strsv( data.order, data.uplo, data.trans, data.diag, 0, a, data.LDA, x, data.strideX );
	t.strictEqual( out, x, 'returns expected value' );
